...
def create_loaders(batch_size, **kwargs):
    ...
    cue = int(len(indices)*4/5)
    train_loader.sampler = SubsetRandomSampler(indices[:cue])

    valid_loader = copy(train_loader)
    valid_loader.sampler = SubsetRandomSampler(indices[cue:])
